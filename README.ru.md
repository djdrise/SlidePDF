...
(<kbd>F5</kbd>), и уходит с глаз, когда показ закончен. Открыть его заранее,
...
assets/icon.ico        16…256 для Windows, собирается здесь же
```

`npm run icon` пересобирает обе картинки из SVG силами Chromium: сторонних
